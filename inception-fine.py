...
def get_model(learning_rate=0.1) -> keras.Model:
...
def train():
    ...
    train_iter, validation_iter, test_iter = get_data_iterator(batch_size=batch_size)

    model = get_model(learning_rate=learning_rate)

    checkpoint = WandbModelCheckpoint(
        os.path.join(MODELS_DIR, FINE_NAME),
        monitor="loss",
        verbose=1,
        save_best_only=True,
    )
    ...
    model = tf.keras.models.load_model(os.path.join(MODELS_DIR, FINE_NAME))
